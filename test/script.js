class WaterRipple {
  ...
  init(props) {
    this.boxRef = document.getElementById(props.boxRef);
    ...
    this.background.src = props.background;
    ...
    this.background.onerror = () => {
      console.error("Failed to load background image", this.background.src);
    };
    ...
    this.bridge.src = props.bridge;
    this.bridge.onload = () => {
      console.log("Bridge image loaded successfully");
    };
    this.bridge.onerror = () => {
      console.error("Failed to load bridge image", this.bridge.src);
    };
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const waterBg = "images/canvas/water.png";
  const bridgeImg = "images/canvas/bridge.png";
  ...
  if (box && canvas) {
    ...
    waterImg.onload = () => {
      console.log("Water background image loaded successfully");
    ...
    waterImg.onerror = () => {
      console.error("Failed to load water background image");
    };
  }

  window.addEventListener("beforeunload", () => {
    if (timer) clearInterval(timer);
    if (waterRipple) waterRipple.stop();
  });
});
